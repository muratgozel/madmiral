...
const {typekit, objectkit, validationkit, arraykit, stringkit} = require('basekits')
...
Notifly.prototype.sendEmail = function sendEmail(params, cb) {
  ...
  msg.setAttachments(objectkit.getProp(params, 'attachments'))
...
Notifly.prototype.runServices = function runServices(channel, msg, cb) {
  ...
  const sorted = arraykit.sortItemsBy(services, 'priority', 'asc')
  const errors = []
